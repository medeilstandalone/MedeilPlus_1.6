...
public class Autobackup {
    ...
    static String[] ignoreTable = {"medeil_plus.drug_temp", "medeil_plus.banned_drug", "medeil_plus.countries", "medeil_plus.state", "medeil_plus.license_details", "medeil_plus.therapeutics", "medeil_plus.formulations", "medeil_plus.user_info", "medeil_plus.user_login"};

    public static void usingDump() {
        String dbName = "medeil_plus";
        String dbUser = "root";
        ...
        try {
            ...
            String executeCmd = "";
            boolean count = false;
            File absPath = new File(DriveTypeInfo.getAutoBackupPath() + "MedeilPlus\\");
            if (!absPath.exists()) {
                absPath.mkdirs();
            }
            ...
            executeCmd = "Data\\bin\\mysqldump -u" + dbUser + " -p" + temp + " " + "--no-create-info "
                    + "--no-create-db --replace --complete-insert --extended-insert --skip-triggers "
                    + "--skip-disable-keys --skip-set-charset --skip-add-locks --no-set-names "
                    + "--quick " + getIgnoreTable() + " " + dbName + " -r \"" + fileName + "\"";
