...
//        }
    ...
    function saveHSN(): Void {
        try {
            ...
            {
                FXinfo("Please Enter HSN Code");
            } 
            else
            {
                var commonController: CommonImplements = RegistryFactory.getClientStub(RegistryConstants.CommonImplements) as CommonImplements;
                var a = commonController.getAddHSN(hsnCode,generic);
                if (a == 1)
                {
                  FXinfo("HsnCode Already Exist");
                    txtHsnCode.requestFocus();
                     Hide();
                } 
                else
                {
                    FXinfo("HsnCode Added Sucessfully");
                    txtHsnCode.text = "";
                    Hide();
                }          
            }
