...
public class AddServCategory {
    ...
    function saveformulation(): Void {
        try { 
         ...
         if(ms==null and ms==""){
              FXinfo("Please Enter Category ");
         }else if(m<=0){
              FXinfo("Please Enter Category ");

         }else if(ms.length()<3){
             FXinfo("Category Contains Minimum 3 Characters");
         }
         else{
              //var commonController:CommonImplements = new CommonController();
               var commonController: CommonImplements=RegistryFactory.getClientStub(RegistryConstants.CommonImplements) as CommonImplements;
                 var a= commonController.getAddServcateg(ms);
                 if (a == 1) {
                    FXinfo("Category is Already Exists.");
