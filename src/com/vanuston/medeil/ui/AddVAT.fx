...
public class AddVAT {
    ...
    def __layoutInfo_txtVAT: javafx.scene.layout.LayoutInfo = javafx.scene.layout.LayoutInfo {
        width: 115.0
    }
    public-read def txtVAT: javafx.scene.control.TextBox = javafx.scene.control.TextBox {
        layoutX: 187.0
        layoutY: 75.0
        layoutInfo: __layoutInfo_txtVAT
...
//        }
    ...
    function saveVAT(): Void {
        try {
            var valObj: Validation = new Validation();
            var ms: String = txtVAT.rawText.trim();
            var m: Integer = 0; m = valObj.getDecimalValid(txtVAT.rawText.trim(), 1, 8);
            if (ms == null and ms == "") {
                FXinfo("Please Enter VAT Value");
            } else if (m != 10) {
                FXinfo("Please Enter Valid VAT Value.(Only Numbers Allowed)");
            } else {
                  var commonController: CommonImplements = RegistryFactory.getClientStub(RegistryConstants.CommonImplements) as CommonImplements;
                var a = commonController.getAddVAT(ms);
                if (a == 1) {
                    FXinfo("VAT Value is Already Exists.");
                    txtVAT.requestFocus();
                } else {
                    VAT = txtVAT.rawText;
                    txtVAT.text = "";
            ...
            //                var a = 0;
            //                var rs: ResultSet = DBConnection.getStatement().executeQuery("select * from vat_master where vat_val='{ms}'");
            //                while (rs.next()) {
            //                    a = 1;
            //                }
            //                if (a == 1) {
            ...
            //                } else {
            //                    VAT = txtVAT.rawText;
            //                    DBConnection.getStatement().executeUpdate("insert into vat_master(vat_val) values('{VAT}')");
            //                    txtVAT.text = "";
            //                    Hide();
            //                }
            }
