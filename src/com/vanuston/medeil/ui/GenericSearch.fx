...
public class GenericSearch {
    ...
    public-read def label5: javafx.scene.control.Label = javafx.scene.control.Label {
        layoutX: bind C2LX+540
        layoutY: 30.0
        text: "(Please select the items per page)"
        font: Arial_Bold_12
        textFill: DarkGray
    }
    ...
    public-read def panelpage: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        ...
        content: [ panelTable, rectTableBorder, labelSearchType, labelGenericName, labelProductName, btnSearch, panelAtoZ, txtGenericName, rdoGeneric, rdoProduct, txtProductName, btnReset, imageDownGeneric, imageDownProduct, labelSearchRecrord, listGeneric, rdoStock, rdoDrug, label2, choicebox, labelPrevious, count1, label4, labelNext, count2, label5, ]
    }
...
function digitCount(number: Integer): Integer {
    ...
    function nextButtonClicked(event: javafx.scene.input.MouseEvent): Void {
        updateIndices(1);
        searchCount = "{genericSearch.LoadGenericSearchResult(drugModel,start,limit)} Records Found";
         if (not searchCount.equals("0 Records Found")) {
...
//            btnSearch.requestFocus();
    ...
    function getGenerics(type:Integer): Void {
       try{
        ...
        var selectItem: String = bind listControlGen.selectedString on replace {
            ...
            var msg: String = "Class : GenericSearch  Method : getGenerics()  = {e.getMessage()}";
            log.debug(msg);
        }
    }
    ...
    function getProducts(type:Integer): Void {
       try{
        ...
        var selectedItem: String = bind listControlPro.selectedString on replace {            
            ...
            var msg: String = "Class : GenericSearch  Method : getProducts()  = {e.getMessage()}";
            log.debug(msg);
        }
    }
    ...
    function findLimit(n: Integer): Integer {
     ...
     function FXinfo1(msg: String): Void {
        CustomAlert.ShowInfo("Search", msg);
        showAlertbox();
    }
    ...
    function btnSearchAction(): Void {
       if(rdoGeneric.selected and not txtGenericName.text.equals("") or rdoProduct.selected and not txtProductName.text.equals(""))
{
    ...
    //   searchCount = "{genericSearch.LoadGenericSearchResult(drugModel)} Records Found";
       searchCount = "{genericSearch.LoadGenericSearchResult(drugModel,0,limit)} Records Found";
