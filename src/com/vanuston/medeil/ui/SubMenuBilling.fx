...
public class SubMenuBilling {

    //sabarish   def SalesCash = SalesCash {};
    //sabarish   def SalesCounterBill = SalesCounterBill {};
    //sabarish   def SalesDummy = SalesDummy {};
    //sabarish   def SalesReturn = SalesReturn {};
    //sabarish   def SalesMaintanance = SalesMaintanance {};
    def SalesCash1 = SalesCash12 {};
    def SalesCash11=Advanceamount{};
    ...
    public-read def group7: javafx.scene.Group = javafx.scene.Group {
        visible: bind DisableSales[5]!=true
        disable: bind DisableSal[5]
    ...
    public-read def panelSalesSubMenu: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        ...
        layoutX: 0.0
        layoutY: 0.0
        content: [ rectSubMenuBg, group1, group2, group3, group4, group5, group6, group7, panelSalesForm, imgClose, ]
    }
                ...
                keyFrames: [
                    javafx.animation.KeyFrame {
                        ...
                        action: function() {
                            ...
                            imgMenu9.visible = false;
                            imgMenu9.layoutX = 16.0;
                            imgMenu9.layoutY = 3.5;
                            imgMenu9.image = imageMaintanance;
                            group6.layoutX = 4.0;
                            rectMenu10.visible = true;
                            rectMenu10.layoutX = 0.0;
                            rectMenu10.layoutY = 0.0;
                            rectMenu10.styleClass = "LeftmenuHighlight";
                            rectMenu10.onMouseClicked = rectMenu10OnMouseClickedAtSubMenu;
                            rectMenu10.fill = null;
    ...
    function rectMenu10OnMouseClickedAtSubMenu(event: javafx.scene.input.MouseEvent): Void {
     //SalesCash12();
     salesCash();
         }
...
//    deleteForm(10);
////saba            HomePage.hide();
//            insert SalesCash1.getDesignRootNodes() into panelSalesForm.content;
//            Advanceamount.StartUp();
//       // }
//    }

 public function salesCash11(): Void {
      if(CommonDeclare.user!="DefaultUser"){
        if(DisableSales[1]){
            Msgbox("Information", ModuleRestrict_Msg);
        }else{
            deleteForm(2);
//              insert Advanceamount.getDesignRootNodes() into panelSalesForm.content;
//               Advanceamount.StartUp();
        }
      }
    }
    ...
    public function salesMaintain(): Void {
        if(DisableSales[4]){
            ...
            deleteForm(8);
        //sabarish       insert SalesMaintanance.getDesignRootNodes() into panelSalesForm.content;
         //sabarish      SalesMaintanance.StartUp();
        }
    }
    ...
    public function salesReturn(): Void {
        if(DisableSales[3]){
            ...
            deleteForm(7);
           //sabarish    insert SalesReturn.getDesignRootNodes() into panelSalesForm.content;
          //sabarish     SalesReturn.startUp();
        }
    }
...
if(DisableSales[0]){
            ...
            deleteForm(1);
            def Advanceamount  = Advanceamount{};
    ...
    public function salesCounter(): Void {
        if (commonController.getProductType().equals("Free")) {
            ...
            deleteForm(2);
           //sabarish    insert SalesCounterBill.getDesignRootNodes() into panelSalesForm.content;
            //sabarish   SalesCounterBill.startUp();

        }
    }

public function SalesCash12(): Void {
        /*if (commonController.getProductType().equals("Free")) {
            editionAlert();
        } */
//         if(DisableSales[0]){
...
//saba            HomePage.hide();
    ...
    function getPrivillages(uname: String): String[] {
        ...
        if (not GetUserInfo.checkName(uname)) {
            var access: String = comObj.getPrivileges("billing", uname);
