...
public class AddTher {

    var log: Logger = Logger.getLogger(AddTher.class, "Utilities");
    public var therapetic = "";
    def CustomAlert = CustomAlert {};
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:main
    def __layoutInfo_txtTherpateic: javafx.scene.layout.LayoutInfo = javafx.scene.layout.LayoutInfo {
        width: 250.0
    }
    public-read def txtTherpateic: javafx.scene.control.TextBox = javafx.scene.control.TextBox {
        layoutX: 187.0
        layoutY: 75.0
        layoutInfo: __layoutInfo_txtTherpateic
        onKeyPressed: txtTherpateicOnKeyPressed
    }
    
    public-read def vbox2: javafx.scene.layout.VBox = javafx.scene.layout.VBox {
        layoutX: 130.0
        layoutY: 30.0
        content: [ txtTherpateic, ]
        spacing: 20.0
    }
    ...
    function FXinfo(msg: String): Void {
        CustomAlert.ShowInfo("Add therapetic", msg);
        showAlertbox();
    }
    ...
    function txtTherpateicOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
    ...
    function savetherapetic(): Void {
        try { 

         var valObj: Validation = new Validation();
         var ms:String=txtTherpateic.rawText.trim();
         var m: Integer = txtTherpateic.rawText.length();
         if(ms==null and ms==""){
              FXinfo("Please Enter therapetic ");
         }else if(m<=0){
              FXinfo("Please Enter therapetic ");
         ...
         var commonController: CommonImplements=RegistryFactory.getClientStub(RegistryConstants.CommonImplements) as CommonImplements;
                 var a= commonController.getAddTherapeutic(ms);
                 if (a == 1) {
                    FXinfo("Therapetic is Already Exists.");
                    txtTherpateic.requestFocus();
                } else {
                     therapetic = txtTherpateic.rawText;
                     txtTherpateic.text = "";
                    Hide();
                    }
//            var a = 0;
//            var rs: ResultSet = DBConnection.getStatement().executeQuery("select * from therapeutics where therapeutic_class='{ms}'");
//            while (rs.next()) {
//                  a = 1;
//
//            }
...
//                } else {
//                    therapetic = txtTherpateic.rawText;
//                    DBConnection.getStatement().executeUpdate("insert into therapeutics values('{therapetic}','')");
//                    txtTherpateic.text = "";
//                    Hide();
//                }
            ...
            var msg: String = "Class : AddCategory  Method : savetherapetic()   = {e.getMessage()}";
            log.debug(msg);
        }
    }
