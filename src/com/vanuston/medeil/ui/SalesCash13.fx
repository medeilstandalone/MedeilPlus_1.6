...
//sabarish var SalesFormTitle:String="Sales Invoice-1 - ctrl+S";
var SalesFormTitle:String="Diagnostic Form";//sabarish
var log: Logger = Logger.getLogger(SalesCash13.class,"SalesCash");
...
def MessageBox=MessageBox{};
    ...
    var frm : Diagnosticdoctor=null;

  //   var form1 : ServiceBilling;
    function btnAddActionAtShown(): Void {
       
  
    var productType = commonController.getProductType();     
           if(count<20 and (productType.equals("Ultimate")or productType.equals("Trial")) ){//sabarish productType.equals("Standard") changed to productType.equals("Premium") //Ultimate
          
            panel.disable = false;
            delete panel.content;
           

            form[count]=Diagnosticdoctor{};
...
//                    }
    ...
    function Load(index:Integer): Void { //On the second form onwards        
        try {
            ...
            SalesFormTitle = "Diagnostic Form-{n + 1} - ctrl+S";//sabarish
            insert form[index].getDesignRootNodes() into panel.content;
            form[index].StartUp();
         } catch (e)
         {
             log.debug(" Class : SalesCash13   Method: Load()   Exception : {e.getMessage()}");
         }
    }
 var opacClose=0.5;
 var fadeClose = Timeline {
    ...
    function cancelForm(): Void {
        try {
            ...
            log.debug(" Class : SalesCash13   Method: cancelForm()   Exception : {e.getMessage()}");
        }
    }
  ...
  public function StartUp(): Void {
      ...
      {
        //  java.lang.System.out.println("inside salescash13 startup");
          
         freeRestrict();
        btnAdd.requestFocus();
        //  java.lang.System.out.println("inside salescash13 startup cc{count}");
        if(count == 0) {
       //    java.lang.System.out.println("inside salescash13 startup if{count}");
            panelBg.visible=true;
            delete panel.content;
             frm.StartUp();//sabarish
            btnAddActionAtShown();
        ...
        //s catch (e: Exception) {
        //s    log.debug("Class : SalesCash  Method: StartUp12()  Exception : {e.getMessage()}");
        //s    java.lang.System.out.println("checking{e}");
        //s     java.lang.System.out.println("checking1{e.getMessage()}");
       //s }
//        log.debug("SalesCash", "startUp", "AE");
