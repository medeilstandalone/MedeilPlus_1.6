...
public class HospitalInformation {
    ...
    public-read def gstno: javafx.scene.control.Label = javafx.scene.control.Label {
        visible: false
        layoutX: bind C3LX
        text: "Label"
    }
    
    def __layoutInfo_txtgstno: javafx.scene.layout.LayoutInfo = javafx.scene.layout.LayoutInfo {
        width: bind Type1TextboxW
    }
    public-read def txtgstno: javafx.scene.control.TextBox = javafx.scene.control.TextBox {
        visible: false
        layoutX: bind C4LX
        layoutInfo: __layoutInfo_txtgstno
    }
    ...
    public-read def rectShopInfo: javafx.scene.shape.Rectangle = javafx.scene.shape.Rectangle {
        visible: false
        width: 100.0
        height: 50.0
    }
    
    public-read def lblShopInfo: javafx.scene.control.Label = javafx.scene.control.Label {
        visible: false
        text: "Label"
    }
    ...
    public-read def group: javafx.scene.Group = javafx.scene.Group {
        visible: false
        layoutX: bind panelW * 82.42 / 100
        layoutY: bind panelH * 92.66 / 100
        content: [ rectShopInfo, lblShopInfo, ]
    }
    ...
    public-read def panelButtons: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        ...
        content: [ btnHSPSave, btnHSPReset, ]
    }
    ...
    public-read def panelGeneral: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        ...
        content: [ label15, separator, label2, txtHSPHName, label9, txtHSPEst, label8, txtHSPGroup, label3, txtHSPHq, gstno, txtgstno, ]
    }
    ...
    public-read def panel: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        ...
        layoutX: 0.0
        layoutY: 0.0
        content: [ rectangle, rectHeader, LblHeader, panelGeneral, panelContact, panelPerson, panelButtons, listCustCountry, listCountry, listCustState, listHospitalName, listState, txtHSPHCode, panelAlert, group, ]
    }
                ...
                keyFrames: [
                    javafx.animation.KeyFrame {
                        ...
                        action: function() {
                            ...
                            gstno.text = "GST NO";
                            gstno.font = Arial_12;
                            gstno.textFill = DarkGray;
                            txtgstno.visible = true;
                            txtgstno.layoutY = 158.0;
                            txtgstno.promptText = "Enter your GST Number";
                            txtgstno.font = Arial_12;
                            ...
                            rectShopInfo.visible = false;
                            rectShopInfo.cursor = javafx.scene.Cursor.HAND;
                            rectShopInfo.styleClass = "rectbutton";
                            rectShopInfo.onKeyPressed = rectShopInfoOnKeyPressedAtShown;
                            rectShopInfo.onMouseClicked = rectShopInfoOnMouseClickedAtShown;
                            rectShopInfo.focusTraversable = true;
                            rectShopInfo.fill = javafx.scene.paint.Color.YELLOW;
                            rectShopInfo.stroke = DarkGray;
                            rectShopInfo.strokeWidth = 2.0;
                            rectShopInfo.width = 120.0;
                            rectShopInfo.height = 20.0;
                            rectShopInfo.arcWidth = 10.0;
                            rectShopInfo.arcHeight = 10.0;
                            lblShopInfo.visible = false;
                            lblShopInfo.layoutX = 10.0;
                            lblShopInfo.layoutY = 2.0;
                            lblShopInfo.styleClass = "labelFormHeading";
                            lblShopInfo.text = "Shop Information";
                            lblShopInfo.font = Arial_Bold_12;
                            lblShopInfo.textFill = Green;
                            group.visible = false;
                            panel.visible = true;
                            panel.opacity = 1.0;
    ...
    public function LoadShopInformation() : Void {
        delete panel.content;
        def sh = ShopInformation {};
        sh.StartUp();
        insert sh.panel into panel.content;
    }

    function rectShopInfoOnMouseClickedAtShown(event: javafx.scene.input.MouseEvent): Void {
        LoadShopInformation();
    }

    function rectShopInfoOnKeyPressedAtShown(event: javafx.scene.input.KeyEvent): Void {
    ...
    def botton1LX = bind CommonDeclare.botton11LX + 120;
    def botton2LX = bind CommonDeclare.botton12LX + 120;
    ...
    function SaveClick(): Void {
        ...
        try {
            ...
            def hospitalModel: HospitalModel = new HospitalModel();
            def san: Integer = valClick();
            if (san == 10) {
                ...
                hospitalModel.setGstNO(txtgstno.rawText.trim().replace("'", ""));
    ...
    def changPiv = bind btnHSPSave.text on replace {
        getPrivileges(logUser);
    }

    function displayHospitalName(): Void {
        try {
            clearHospital();
            //Controller Instance
            hospitalController = RegistryFactory.getClientStub(RegistryConstants.Hospital) as Hospital;
            var hospitalModel: HospitalModel = hospitalController.viewRecord("") as HospitalModel;
            hospitalName = " {hospitalModel.getHospitalName()}";
            ...
            txtgstno.text = hospitalModel.getGstNO();
