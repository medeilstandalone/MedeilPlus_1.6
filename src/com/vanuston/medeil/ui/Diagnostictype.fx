...
public class Diagnostictype {
    ...
    var log: Logger = Logger.getLogger(Diagnostictype.class, "Utilities");
    ...
    function saveManufacturer(): Void {
        try { 
         ...
         if(ms==null or ms=="" or m<=0){
             ...
             FXinfo("Diagnostic Type should contain Minimum 3 Characters");
         }
         else{
               var commonController: CommonImplements=RegistryFactory.getClientStub(RegistryConstants.CommonImplements) as CommonImplements;
               var mfrExists= commonController.getQueryValue("select count(*) from labreportcategory where catagoryname = '{txtManufacturer.rawText.trim()}'");
                 if (mfrExists.equals("0")) {
                     var save= commonController.queryExecution("insert into labreportcategory(catagoryname,status) values ('{txtManufacturer.rawText.trim()}',0)");
                     if(save > 0) {
                     ...
                     FXinfo("Diagnostic Type already Exists.");
                     txtManufacturer.requestFocus();
                    }
            }
        } catch (e: Exception) {
            var msg: String = "Class : Diagnostictype  Method : saveManufacturer()   = {e.getMessage()}";
            log.debug(msg);
        }
    }
