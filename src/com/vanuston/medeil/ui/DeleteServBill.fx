...
import com.vanuston.medeil.ui.MessageBox;

public class DeleteServBill {
    ...
    def __layoutInfo_txtHsnCode: javafx.scene.layout.LayoutInfo = javafx.scene.layout.LayoutInfo {
        width: 154.0
    }
    public-read def txtHsnCode: javafx.scene.control.TextBox = javafx.scene.control.TextBox {
        layoutX: 187.0
        layoutY: 0.0
        layoutInfo: __layoutInfo_txtHsnCode
        onKeyPressed: txtVATOnKeyPressed
        onKeyTyped: txtVATOnKeyTyped
    }
    ...
    def __layoutInfo_textgenericname: javafx.scene.layout.LayoutInfo = javafx.scene.layout.LayoutInfo {
        width: 148.0
    }
    public-read def textgenericname: javafx.scene.control.TextBox = javafx.scene.control.TextBox {
        visible: false
        layoutX: 136.0
        layoutY: 2.0
        layoutInfo: __layoutInfo_textgenericname
    }
    ...
    public-read def ItemName: javafx.scene.control.Label = javafx.scene.control.Label {
        visible: false
        layoutX: 30.0
        layoutY: 2.0
        text: "Description"
    ...
    public-read def panelLicenceInfo: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        layoutX: 0.0
        layoutY: 30.0
        content: [ label9, vbox, label20, vbox1, vbox2, hbox, hbox2, ItemName, textgenericname, ]
    }
    ...
    function txtVATOnKeyTyped(event: javafx.scene.input.KeyEvent): Void {
//        if (event.code == KeyCode.VK_ENTER or event.code == KeyCode.VK_TAB) {
//            if (not isNumeric(txtVAT)) {
//                txtVAT.requestFocus();
//            }
//        }
    }
    ...
    function FXinfo(msg: String): Void {
        CustomAlert.ShowInfo("Add VAT", msg);
        showAlertbox();
    }
    ...
    function txtVATOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
        if (event.code == KeyCode.VK_ENTER or event.code == KeyCode.VK_TAB) {
            button.requestFocus();
        }
    }
    ...
    function saveHSN(): Void {
        try {
            var valObj: Validation = new Validation();
            var generic:String=textgenericname.rawText.trim();
            var hsnCode: String = txtHsnCode.rawText.trim();
            var m: Integer = 0; m = valObj.getAlphaNumValid(txtHsnCode.rawText.trim(), 1, 8);
            if (hsnCode == null and hsnCode == "")
            {
                FXinfo("Please Enter Bill No");
            } 
            else
            {
                var commonController: CommonImplements = RegistryFactory.getClientStub(RegistryConstants.CommonImplements) as CommonImplements;
                var a = commonController.getDeleteServBill(hsnCode);
//                FXinfo("Bill Deleted Sucessfully");
...
//            FXinfo("Bill Deleted Sucessfully");
            ...
            var msg: String = "Class : AddCategory  Method : saveVAT()   = {e.getMessage()}";
            log.debug(msg);
        }
    }
...
//    }
    ...
    public function Hide(): Void {
        ...
        fadePanel.play();
        panel.visible = false;
//        FXinfo("Bill Deleted Successfully");
    }
