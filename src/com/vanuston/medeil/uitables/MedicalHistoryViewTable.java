...
public class MedicalHistoryViewTable extends javax.swing.JDialog {

    Logger log = Logger.getLogger(MedicalHistoryTable.class, "MedicalHistoryTable");
    public JTable jcomp;
    ...
    final int[] colWidth = {40, 120, 60, 60, 60, 0, 60, 60, 60, 60, 50};
    ...
    public JComponent createTable() {
       ...
       final boolean[] canEditCols = {false, false, false, false, false, false, false, false, false, false};
    ...
    public List<MedicalHistoryModel1> getTablevalue(Object Mhm1) {
       // System.out.println("inside getTablevalue");
        int rowCount = jcomp.getModel().getRowCount();
        List<MedicalHistoryModel1> values = new ArrayList<MedicalHistoryModel1>();
        MedicalHistoryModel1 model;
        model=(MedicalHistoryModel1)Mhm1;
        values=model.getPrescriptionListItems();
        try {
          
             for(int i=0;i<values.size();i++) {
                model = values.get(i);
                jcomp.getModel().setValueAt(i+1, i, 0);
                jcomp.getModel().setValueAt(model.getMedicineName(), i, 1);
                jcomp.getModel().setValueAt(model.getMorning(), i, 2);
                jcomp.getModel().setValueAt(model.getAfternoon(), i, 3);
                jcomp.getModel().setValueAt(model.getEvening(), i, 4);
                jcomp.getModel().setValueAt(model.getNight(), i, 5);
                jcomp.getModel().setValueAt(model.getFood(), i, 6);
                jcomp.getModel().setValueAt(model.getDose(), i, 7);
                jcomp.getModel().setValueAt(model.getTotalMedications(), i, 8);
                jcomp.getModel().setValueAt(model.getDays(), i, 9);
                this.dispose();
                jcomp.requestFocus();
                jcomp.changeSelection(i, 8, false, false);
                //sabarish jcomp.setEditingColumn(4);
         }
        } catch (Exception ex) {
    ...
    public void resetValues() {
     ...
     public void saleshide(){
        jcomp.getColumnModel().getColumn(6).setMinWidth(0);
        jcomp.getColumnModel().getColumn(6).setMaxWidth(0);
    }
