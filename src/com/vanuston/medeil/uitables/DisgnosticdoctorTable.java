...
public class DisgnosticdoctorTable extends javax.swing.JDialog {
...
//sabarish    public  GetSubstituteDrugs sd;
    ...
    //                       0   1   2    3   4  5   6
    final int[] colWidth = {20, 250,250};//sabarish
    ...
    public JComponent createTable() {
        ...
        //                    0          1               2           3               4         5             6
        //Object[] colName = {"S.No", "Particulars", "Qty/Days", "Discount", "Price/per unit", "Vat", "Total Amount"};
        //                    0          1             2
        Object[] colName = {"S.No", "Report Name", "Category"};
        thisForm = "CashSales";
        // padhu   //initR = 15;
        initR = 100;
        //                               0                        1                      2                     Amount-index 12,Adjid index 13
        final Class[] types = {java.lang.Integer.class, java.lang.String.class, java.lang.String.class};
        ...
        //  sabarish                    0       1      2     3     4     5      6
        final boolean[] canEditCols = {false, false, false};
        ...
        try {
            ...
            jcomp.addKeyListener(new KeyAdapter() {
                ...
                public void keyReleased(KeyEvent e) {
                    ...
                    final int j = jcomp.getSelectedColumn();
                    try {
                        if (i == jcomp.getModel().getRowCount() - 2) {
                            addRow();
                        }
                       // salesCalculations();
                        if (e.getKeyCode() == 10) {
                        ...
                        if (j == 2) {
                            ...
                            int stkQty = 0;
                            stkQty = Integer.parseInt(salesController.getStockQty(jcomp.getModel().getValueAt(i, 2).toString(), jcomp.getModel().getValueAt(i, 2).toString()));
                            int qty = 0;
                ...
                public void keyPressed(KeyEvent e) {
                    ...
                    try {
                        ...
                        if (e.isActionKey()) {
                            for (int val = 0; val < jcomp.getRowCount(); val++) {
                                if (jcomp.getModel().getValueAt(val, 2) == null); else {
                                    jcomp.getModel().setValueAt(val + 2, val, 0);
                                }
                                if (jcomp.getModel().getValueAt(val, 2) == null); else {
    ...
    public List<DiagnosticdoctorModel> getTableValues() {
        int rowCount = jcomp.getModel().getRowCount();
        List<DiagnosticdoctorModel> values = new ArrayList<DiagnosticdoctorModel>();
        DiagnosticdoctorModel model;
        try {
            for (int i = 0; i < rowCount; i++) {
                    model = new DiagnosticdoctorModel();
                    if (jcomp.getModel().getValueAt(i, 2) == null && jcomp.getModel().getValueAt(i,2) == null); else {
                        model.setLabreportname(""+jcomp.getModel().getValueAt(i, 1));

                         model.setCatoryname(""+jcomp.getModel().getValueAt(i, 2));
                ...
                if (i + 2 != rows) {
                    for (int n = 0; n < rows1; n++) {
                        if (jcomp.getModel().getValueAt(n, 0) == null) {
                            jcomp.getModel().setValueAt(2, 0, 0);
                        } else {
                            jcomp.getModel().setValueAt(n + 2, n, 0);
                        }
                    }
                    jcomp.changeSelection(0, 0, false, false);
                    ...
                    jcomp.changeSelection(0, 0, false, false);
                    jcomp.changeSelection(i - 2, j, false, false);
                }
            }
            //sabarish  salesCalculations();
        } else {
            ...
            try {
                for (int index = 0; index < jcomp.getRowCount(); index++) {
                    if (!(jcomp.getModel().getValueAt(index, 2) == null)) {
                        ...
                        model.setItemCode(jcomp.getModel().getValueAt(index, 2).toString());
               ...
               // System.out.print("diagnosticdoctor table (2)=" + e);
                String ss = "Method: getSalesBillItems Exception : " + e.getMessage();
                log.debug(ss);
            }
        } catch (Exception exception) {
           // System.out.print("diagnosticdoctor table1234 " + exception);
    ...
    public int getDataRowCount() {
        ...
        for (int rowIndex = 0; rowIndex < jcomp.getModel().getRowCount(); rowIndex++) {
            if (jcomp.getModel().getValueAt(rowIndex, 2) != null && jcomp.getModel().getValueAt(rowIndex, 2) != null && jcomp.getModel().getValueAt(rowIndex, 2) != "" && jcomp.getModel().getValueAt(rowIndex, 2) != "") {
            } else {
                rowCount = rowIndex;
                break;
            }
        ...
        try {
            ...
            for (int index = 0; index < salesBillItems.size(); index++) {
                ...
                for (int rowIndex = 0; rowIndex < rowCount && rowCount != 0; rowIndex++) {
                    if (sales.getItemCode().equals(jcomp.getModel().getValueAt(rowIndex, 2).toString()) && sales.getBatchNumber().equals(jcomp.getModel().getValueAt(rowIndex, 2).toString())) {
                        flag = true;
                        break;
                    }
                }
...
//                  }
                if (rowCount != 0 && flag == false) {
                    addRow();
                    jcomp.getModel().setValueAt(rowCount + 2, rowCount, 0);
                   ...
                   // jcomp.getModel().setValueAt(sales.getReturnExpiryDate(), rowCount, 8);
                    ...
                    jcomp.getModel().setValueAt(rowCount + 2, rowCount, 0);
                    jcomp.getModel().setValueAt(sales.getItemCode(), rowCount, 2);
                   // jcomp.getModel().setValueAt(sales.getItemName(), rowCount, 2);
                   // jcomp.getModel().setValueAt(sales.getFormulation(), rowCount, 3);
                   // jcomp.getModel().setValueAt(sales.getQuantity(), rowCount, 4);
                  //  jcomp.getModel().setValueAt(sales.getBatchNumber(), rowCount, 7);
    ...
    public void focusSet() {
        jcomp.requestFocus();
        jcomp.changeSelection(0, 2, false, false);
        jcomp.getModel().setValueAt("2", 0, 0);
        jcomp.setCellSelectionEnabled(true);
    }
...
//ajith work
                ...
                if (jcomp.getModel().getValueAt(i, 2) != null
                    ...
                    if (jcomp.getModel().getValueAt(i, 2) != null
                        ...
                        if (productCode.equals(jcomp.getModel().getValueAt(i, 2) + "")
                                && Ibatchno.equals(jcomp.getModel().getValueAt(i, 2) + "")) {
                            rowVal = i;
                            jcomp.changeSelection(rowVal, 2, false, false);
                            break;
    ...
    public String[] setTablevaluesss(String Dianame,String Item_Name, int sno, int st) {
...
//sql1="select distinct(s.item_name),s.item_code,d.mfgname,d.dosage,s.selling_price,s.mrp,d.vat,s.batch_no,s.formulation,s.ban_flag_id ,s.qty ,s.rack_no, s.shelf_no,s.sales_discount,s.purchase_price,s.selling_price,s.packing,s.expiry_date from stock_statement s, drugtable d where s.item_name='"+Item_Code+"' and s.batch_no='"+Ibatchno+"' and s.ss_flag_id = 0 group by d.vat";
                sql1 = "select c.catagoryname,l.* from labreport_list l left join labreportcategory c on c.catagoryid=l.catagoryid where l.reportname='" + Item_Name + "' and c.catagoryname='" + Dianame + "' ";
            } else {
                sql1 = "select c.catagoryname,l.* from labreport_list l left join labreportcategory c on c.catagoryid=l.catagoryid where l.reportname='" + Item_Name + "' and c.catagoryname='" + Dianame + "' ";
            }
            Sales salesController = (Sales) RegistryFactory.getClientStub(RegistryConstants.Sales);
            
            List<SalesModel> salesList = salesController.getQueryTableValues1(sql1);
            for (int index = 0; index < salesList.size(); index++) {
                ...
                jcomp.getModel().setValueAt(listModel.getReportname(), rowPointer, 1);
                jcomp.getModel().setValueAt(listModel.getCategory(), rowPointer, 2);


               
              //naziya  jcomp.getModel().setValueAt("2", rowPointer, 2);
              //naziya   jcomp.getModel().setValueAt(0, rowPointer, 3);
             //naziya   jcomp.getModel().setValueAt(listModel.getBilling_amount(), rowPointer, 4);
              //naziya   jcomp.getModel().setValueAt(0, rowPointer, 5);
